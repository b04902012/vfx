...
def toMaskImage(img):
    m = img.median()
    return (img>m*1.1 or img<m*0.9)

def shiftImage(img, shift_x, shift_y):
    new_img = img
    for x in range(new_img.shape[0]) :
        for y in range(new_img.shape[1]):
            if(x+shift_x<0 or y+shift_y<0):
                new_img[x,y] = False
                continue
            try:
                new_img[x,y] = img[x+shift_x, y+shift_y]
            except:
                new_img[x,y] = False
    return new_img
def 
        ...
        while(num_shift >= 0):
            ...
            for x in range(-1,2) :
                for y in range(-1,2) :
                    ...
                    img1 = shiftImage(imgs[i], del_x, del_y)
