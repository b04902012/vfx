...
if __name__ == '__main__':
    ...
    if input('Already has radiance map? [y/n] ') in ('y', 'Y'):
        rad_img = hdr.loadHDR(dir_name+'/'+dir_name+'_rad')
    else:
    ...
    if input('Do tone mapping? [y/n] ') in ('y', 'Y'):
        ...
        key_value, multi_value = input('Please input key value and multi value: ').split()
        key_value, multi_value = float(key_value), float(multi_value)
        tone_mapped = hdr.photographic(rad_img, key_value, multi_value)

        print(np.amax(tone_mapped))
        print(np.amin(tone_mapped))

        cv2.imwrite(dir_name+'/'+dir_name+f'_toned_{key_value}_{multi_value}.png',tone_mapped)
